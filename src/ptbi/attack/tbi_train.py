...
def train_paired_ptbi_inv_model(
    prediction_dataloader,
    device,
    inv,
    inv_optimizer,
    ...
    running_size = 0
    for data in prediction_dataloader:
        loss, x = train_paired_inv_model_bw(
            data,
            device,
            inv,
            inv_optimizer,
    ...
    return inv_running_loss / running_size, x
...
def get_paired_inv_train_fn_ptbi(
    client_num,
    is_sensitive_flag,
    local_identities,
    inv_transform,
    ...
    def inv_train(api):
        ...
        for target_client_id in range(client_num):
            ...
            for i in range(1, inv_epoch + 1):
                (inv_running_loss, _) = train_paired_ptbi_inv_model(
                    prediction_dataloader,
                    device,
                    inv,
                    inv_optimizer,
